...
p2psocket.emit('firstCom', 'Hello there. I am ' + p2psocket.peerId);
...
// When told to start rendering
p2psocket.on('startRendering', function(data){
  res.send('<h1>Hello world</h1>');
  // Convert from string to MatterJS object
  var bodies = necromancer.resurrect(data);

  // This function needs to be created
  startRendering(bodies);
});

p2psocket.on('arrayOfSockets', function(data){
  listOfNodes = data;

  // Might not work because of how I send the listOfNodes
  nodeNumber = listOfNodes.indexOf(socket.id);
});

// Figure out what to do when you recieve data
p2psocket.on('dataFromPeer', function(data){
  // Probably find a way to add the data to the world
  var recievedObjects = necromancer.resurrect(data);
});

// Send data to left peer
// At first this will just go through the server
function sendLeft(data){
  sendData(data, nodeNumber - 1);
}

// Send data to right peer
function sendRight(data){
  sendData(data, nodeNumber + 1);
}

// Actually send the data but currently sends it via the server, not p2p
// Once I do p2p, I won't need to additionally add the nodeID
function sendData(data, nodeID){
  p2psocket.emit('sendOnwards', {data: necromancer.stringify(data), id: nodeID});
}

// send data to server
function sendCenter(data){

}

// Start the physical rendering
function startRendering(bodies){
